...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的房屋订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li v-for="item in fangwuOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.fangwuName}}</td>
                            <td style="width: 80px;">{{item.fangwuHuxingValue}}</td>
                            <td style="width: 60px;">{{item.fangwuWeizhi}}</td>
                            <td style="width: 60px;">{{item.fangwuTedian}}</td>
                            <td style="width: 60px;">{{item.fangwuTime}}</td>
                            <td style="width: 250px;">
                                <img :src="item.fangwuPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 100px;color: red">{{item.fangwuNewMoney}}</td>
                            <td style="width: 60px;">{{item.fangwuPingmi}}</td>
                            <td style="width: 60px;">{{item.fangwuNumber}}</td>
                            <td style="width: 80px;">{{item.fangwuOrderValue}}</td>
                            <td style="width: 150px;">{{item.fangwuOrderTime}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 fangwuOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            fangwuOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(fangwuOrderTypes) {
                this.fangwuOrderTypes = fangwuOrderTypes;
                // 获取列表数据
                layui.http.request('fangwuOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    fangwuOrderTypes: vue.fangwuOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('fangwuOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    fangwuOrderTypes: this.fangwuOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`fangwuOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`fangwuOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('fangwuOrder/list', 'get',{
            page: 1,
            limit: limit,
            fangwuOrderTypes: vue.fangwuOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('fangwuOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            fangwuOrderType: vue.fangwuOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
       ...
       function fangwuOrderTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fangwu_order_types", 'get', {}, function (res) {
               if(res.code == 0){
